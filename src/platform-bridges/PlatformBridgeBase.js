...
class PlatformBridgeBase {
                ...
                if (this._localStorage) {
                    if (Array.isArray(key)) {
                        ...
                        for (let i = 0; i < key.length; i++) {
                            values.push(this._getDataFromLocalStorage(key[i], shouldParseValue))
                        }

                        return Promise.resolve(values)
                    }

                    const value = this._getDataFromLocalStorage(key, shouldParseValue)
    ...
    _getDataFromLocalStorage(key, shouldParseValue) {
